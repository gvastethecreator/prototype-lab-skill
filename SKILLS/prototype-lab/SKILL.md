...
1. Confirm lab fit.
   ...
   - If the user asks for several prototypes from one prompt, different models, different skills, A/B options, side-by-side review, or a gallery of alternate executions, mark it as comparison mode. Default to one chronological prototype folder with multiple variants inside it; create separate sibling folders only when the user explicitly needs independently runnable artifacts.
