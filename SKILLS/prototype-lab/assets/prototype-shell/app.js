...
const provenance = {
  ...
  integrity: {
    ...
    lockedDimension: "model comparison | skill comparison | prompt comparison | design options",
    ...
    crossVariantLeakage: "not checked",
...
function renderFocus() {
  ...
  notes.append(
    ...
    labelBlock("Input Scope", variant.inputScope),
    labelBlock("Other Variants Seen", String(variant.receivedOtherVariants)),
...
function renderProvenance() {
  ...
  const rows = [
    ...
    ["Integrity", `${provenance.integrity.sharedBriefId} / ${provenance.integrity.lockedDimension}`],
    ["Allowed Inputs", provenance.integrity.allowedVariantInputs],
    ["Blocked Inputs", provenance.integrity.blockedVariantInputs],
    ["Leakage Check", provenance.integrity.crossVariantLeakage],
    [
      "Agent Runs",
...
function renderControls() {
  ...
  const snapshot = {
    ...
    fit: fitStatus(),
    provenance,
    variants: variants.map(({ id, status, source, agentMode, agentTool, inputScope, receivedOtherVariants }) => ({
      id,
      status,
      source,
      agentMode,
